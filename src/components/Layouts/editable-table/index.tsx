...
export function DataTable<T extends { id: string }>({
  ...
  // Auto-detect columns from data structure
  ...
  const handleSaveConfirmed = () => {
    if (editingId && onEdit) {
      editData?.(data.map(row => row.id === editingId ? editableData as T : row));
  ...
  return (
    <div>
      <div className="flex justify-end my-3">
        <button className="flex max-w-xl justify-center rounded-lg bg-primary p-2 px-6 font-medium text-white hover:bg-opacity-90"
      ...
      {showAddNewForm && (<FormPopup title="Add New" fields={columns.filter(x => x.key != "id")} onOkay={handleAdd} onCancel={() => setShowAddNewForm(false)} />)}
